...
fun Execution.mapPartyToCordaX500ForExecution(serviceHub : ServiceHub) : List<Party>{
    ...
    val client = this.partyRole.filter { it.role == PartyRoleEnum.CLIENT }.map { it.partyReference.globalReference }

    val parties : MutableSet<Party> = mutableSetOf()
    this.party.filter { !client.contains(it.globalReference) }.forEach { parties.add(serviceHub.identityService
...
fun Execution.mapPartyToCordaX500ForAllocation(serviceHub : ServiceHub) : List<Party>{
    ...
    val client = this.partyRole.filter { it.role == PartyRoleEnum.COUNTERPARTY }.map { it.partyReference.globalReference }
